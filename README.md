...
2. Затем в _PostgreSQL_ создаете БД с названием `PasteBin_Project`
    - Введите - _username_, _password_ от своего аккаунта в `application.properites` *(просто измените значение переменных)

3. Потом запускаете _сервер_. Можете сделать это через _IntelliJ IDEA_, или через любой другой удобный для вас _IDE_:
...
4. Как только ваш сервер запустится, используйте этот _URL-адрес_ как основу для построения _API_ запросов `http://localhost:8080/api/v*/pastbin`
